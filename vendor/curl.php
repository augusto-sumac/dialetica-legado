...
class Curl
{
    ...
    {
        // echo pr('CURL - ' . $this->curlOptions['URL']) . "\n";
        ...
        // echo pr(compact('response', 'responseData', 'responseHeader'));
    ...
    protected function parseHeaders($headerString)
    {
        ...
        foreach ($headers as $values) {
            ...
            $key = array_keys((array)$values)[0];
            if (isset($results[$key])) {
                $results[$key] = array_merge(
                    (array) $results[$key],
                    array(array_values((array)$values)[0])
                );
            } else {
                $results = array_merge(
                    $results,
    ...
    protected function returnResponse($content, array $responseData = array(), $header = null)
    {
        ...
        self::$last_response = compact('content', 'responseData', 'header');
